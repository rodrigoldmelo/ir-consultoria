...
- Hotfix de performance: a listagem detalhada voltou a carregar um recorte recente leve, enquanto Dashboard, Inbox, Leads e filtros recebem contadores reais por `count` no Supabase. Assim os números não travam no recorte antigo e a abertura das telas não precisa enriquecer milhares de conversas/leads a cada refresh.
- Hotfix adicional da rota lenta `/api/ir/panel/conversations`: recorte operacional reduzido para 100 conversas recentes e migration `0005_panel_performance_indexes.sql` adiciona índices para `updated_at`, `status`, telefone de leads e última mensagem por conversa.
...
- Ajustado o tom para reduzir repetição do nome do lead: `Dr(a). {Nome}` fica restrito à saudação e, no máximo, um reforço natural. O pedido de documentos também passou a avisar que CNIS e DIRFs/rendimentos são necessários para análise mais precisa, usando o PDF único aprovado.
- Corrigida a classificação de documentos recebidos depois do pedido de CNIS: quando a conversa está aguardando documentos, PDF/imagem/documento sem legenda clara passa a ser tratado como `cnis` por contexto, evitando salvar como `other` e responder que ainda falta CNIS. A confirmação também passou a dizer “Recebi e registrei o CNIS” e encaminhar para triagem humana, citando DIRFs/rendimentos como etapa posterior de precisão.
...
- Ajustado o checklist documental para refletir o fluxo real: CNIS + DIRF/rendimentos como material principal de precisão. O script `fix:cnis-doc` usa `ws` no Node 20 da VPS, reclassifica o documento antigo como CNIS e deixa a conversa aguardando DIRF/rendimentos, não mais “faltando CNIS”.
